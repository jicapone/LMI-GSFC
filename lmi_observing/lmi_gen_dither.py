...
"""
def gen_dither_file( fnout, exptime, filts, nexp, max_dith=20, alt_filts=False ):
	
	fout = open(fnout, 'w')
	
	# write dither header
	fout.write("#title=false ra=false dec=false exposureTime=true numExposures=false filter=true muRA=false muDec=false epoch=false dRA=false dDec=false rotatorPA=false rotatorFrame=false xi=true eta=true comment=true\n#\n")
	...
	if alt_filts:
		for i in range(nexp):
			for j in range(nfilts):
				...
				fout.write(stemp.format(exptime[j], filts[j], dithers[i,0], dithers[i,1], comment))
	else:
		for j in range(nfilts):
			for i in range(nexp):
				comment = '"dXi,Eta = {:5.1f},{:5.1f} {}"'.format(dithers[i,0], dithers[i,1], filts[j])
